    ...
    public class BookService : IBookService
    {
        ...
        public async Task<ServiceResult<PagedResult<Book>>> SearchBooksAsync(BookSearchDto searchDto)
        {
            ...
            if (!validation.IsValid)
                return ServiceResult<PagedResult<Book>>.ValidationFailure(validation.Errors);
        ...
        public async Task<ServiceResult<int>> AddBookAsync(Book book, int userId)
        {
            ...
            if (!validation.IsValid)
                return ServiceResult<int>.ValidationFailure(validation.Errors);
            ...
            if (existing != null)
                return ServiceResult<int>.Failure("يوجد كتاب بنفس الرقم المعياري");
        ...
        public async Task<ServiceResult<bool>> UpdateBookAsync(Book book, int userId)
        {
            ...
            if (!validation.IsValid)
                return ServiceResult<bool>.ValidationFailure(validation.Errors);
        ...
        {
            var errors = new List<string>();
            if (searchDto.PageNumber <= 0) errors.Add("رقم الصفحة يجب أن يكون أكبر من صفر");
